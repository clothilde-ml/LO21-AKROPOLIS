# LO21 — Akropolis (C++)

*This repository was created as part of the **LO21 - OOP** course at UTC (Université de Technologie de Compiègne). It is a group project implementing the Akropolis board game in C++, featuring a console interface and designed to support a Qt GUI, built around a strict MVC architecture.*

---

## 🇫🇷 Présentation

Ce projet est une implémentation en **C++** du jeu de société **Akropolis**, réalisée en groupe de 5 dans le cadre de l'UV LO21. L'application est jouable en mode console et conçue pour supporter une interface **Qt**, autour d'une architecture **MVC** (*Model-View-Controller*).

Ma contribution s'est porté sur deux aspects du projet. D'un côté, la conception de l'architecture globale du projet (définition des couches MVC et gestion des dépendances entre composants), et de l'autre, la modélisation de la grille hexagonale (une structure extensible en hauteur et soumise à des contraintes de placement, qui doit s'afficher de façon intuitive et jouable en console). La réflexion autour de son affichage est documentée dans [`Explications-GrilleHexa-et-Affichage.pdf`](./Explications-GrilleHexa-et-Affichage.pdf).

---

## Collaborateurs

*Projet réalisé avec Elsa Bazile, Axel Drux, Sofia Dulac-Belesgaa et Nyla Junk.*
